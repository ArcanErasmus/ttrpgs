# Sureh's Inventory
## Item List
| Name                        | Weight | Notes
|-----------------------------|        |--------------------------------
| **Worn Equipment**          |        | On body only
|-----------------------------|        |--------------------------------
| Backpack                    |   2    |
| Outfit, Monk's              |   2    |
| Speed Sheath                |   1    | For storing the wand
| Tabard                      |   1    |
|-----------------------------|        |--------------------------------
| **Slotted Magic Items**     |        | See slots below
|-----------------------------|        |--------------------------------
| *Belt of Tumbling*          |   1    | See below
|-----------------------------|        |--------------------------------
| **Weapons/Shields**         |        | Includes ammo, related items
|-----------------------------|        |--------------------------------
| Mwk Handwraps               |        |
|-----------------------------|        |--------------------------------
| **General**                 |        | Non-consumable catch-all
|-----------------------------|        |--------------------------------
| *Wand of Mage Armor*        |        | (UMD DC 20) CL 1, 50/50
| Camp Supplies               |  12    | Bedroll, rations (7)
| Kit, Pathfinder's           |   8    | See below
|-----------------------------|        |--------------------------------
| **Tools**                   |        |
|-----------------------------|        |--------------------------------
| Cushion Inserts             |        | +1 circ Con to continue running
| Elbow and knee pads         |   1    | See below
| Focus Tool, Wand            |   1    | See below
| Kit, Climber's              |   5    | +2 circ to Climb
|-----------------------------|        |--------------------------------
| **Misc/Loot**               |        |
|-----------------------------|        |--------------------------------
|-----------------------------|        |--------------------------------
| **Coins**                   |   ?.?  | $322.9; $0 valuables
|-----------------------------|--------|--------------------------------
| **Total**                   |  34.0  | Light Load

### Gear Descriptions
#### Pathfinder's kit
- Clay mug
- Dagger
- Fishhooks (2)
- Flint & steel
- Sewing needle
- Signal whistle
- String (50ft)
- Thread (50ft)
- Waterskin
- Whetstone

#### Speed Sheath
- Swift to retrieve, full-round to store, both provoke
- Can store dagger/dart/potion/scroll/wand or similar

#### Wand Focus Tool
- Pair of magically resonant woven tokens, affix one to the user's wrist and one to a wand, full-round action
- Grants the user +2 circimstance to UMD checks to activate the wand
- Can only wear one at a time

#### Elbow and knee pads (GM authorized)
- Help with rolling aggressively
- Grants the wearer +2 circumstance to Acrobatics checks to avoid opportunity attacks when moving through threatened squares or opponents' spaces

#### *Daredevil Softpaws Boots*
- Free action, click heels together to activate, max 10rds/day
    - +5 comp to Acro to avoid oppys
- If avoid oppy, +2 bonus atk rolls vs that enemy

### Spent (6000 total)
0 - Free outfit
5 - Tabard
10 - Speed Sheath
12 - Pathfinder's Kit
50 - Wand Focus Tool
50 - Elbow and Knee Pads
80 - Climber's Kit
500.1 - Masterwork Handwraps, Cryptstone
750 - *Wand of Mage Armor* (CL 1)
1020 - HP Retraining
1400 -  *Daredevil Softpaws Boots*
2000 - *Ring of Protection +1*
=5477.1


## Carrying Capacity
| Load      | lbs | Penalties
|-----------|-----|------------
| Light     | 100 |
| Medium    | 200 | +3 max dex to AC, -3 ACP
| Heavy     | 300 | +1 max dex to AC, -6 ACP, x3 run
| Lift      | 600 | No dex to AC, 5 ft move as full-round action
| Push/Drag |1500 |
|-----------|-----|

## Magic Item Slots
- Head      -
- Headband  -
- Eyes      -
- Shoulders -
- Neck      -
- Chest     -
- Body      -
- Armor     -
- Belt      -
- Wrists    -
- Hands     -
- Ring 1    - *Ring of Protection +1*
- Ring 2    -
- Feet      - *Daredevil Softpaws Boots*

## Loot pile
- 
