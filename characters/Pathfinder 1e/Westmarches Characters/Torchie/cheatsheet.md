# Dayflower's Abilities Cheat-sheet
## Weapons
| Weapon                | Atk    | Damage               | Crit     | Range | Notes
|-----------------------|--------|----------------------|----------|-------|------------
| Founderflame Torch    | +10    | 2d6+4(B)+1d4(Fire)   |    20/x2 |       | On hit, -1 saves vs fire dmg effects for 1rd
|   Off-Hand TWF        |(-2/-2) | /2d6+2(B)+1d4(Fire)  |    20/x2 |       | On hit, -1 saves vs fire dmg effects for 1rd
| Anytool Sledge        | +10    | 4d6+6(B)             |    20/x2 |       |
| Improvised Melee      | +10    | 2d6+4(B/P/S)         |    20/x2 |       | +2 dmg if 2h
| Improvised Thrown     | +5     | 2d6+4(B/P/S)         |    20/x2 |       |
|-----------------------|--------|----------------------|----------|-------|------------

## Combat Options
- Blessings (DC 14 if relevant)
    - **Daily**; 7/day: OOOOO OO
    - `Healing`: Swift action, Empower (50% more numerical effects) cure spell as being cast
    - `Protection`: Standard action, +1 sacred to AC and saves for self 1 min
- Sacred Weapon +1
    - **Daily**; 4 rds/day: OOOO
    - Swift action enhance one wpn with *+1*
    - If already +1, can instead give *defending*, *flaming*, *frost*, *ghost touch*, *keen*, *shock*
    - First use locks in blessing configuration for rest of day
- Fervor (DC 14 if relevant)
    - **Daily**; 4/day: OOOO
    - Positive energy (heal living/harm undead no save) 1d6
        - Standard on others, swift on self
    - Swift action, fast cast (1 rd casting or less) prepared spell
        - Only targets self, doesn't provoke, ignore somatic components, need no free hand
    - Standard action, 2 points, 30-ft radius burst heal/harm (pick) (save for half)
- Shikigami Style
    - Swift action to enter, improvised wpns deal dmg as if 2 sizes larger
    - Can take -2 to hit to treat an improvised weapon as having a quality until start of turn:
        - Blocking, brace, disarm, distracting, nonlethal, performance, trip
        - If fragile quality or broken condition, can treat as if jagged quality
- TWF
    - Extra attack with off-hand weapon whenever full-attack action
    - All attacks during this action at -2 (-4 if off-hand not light)

## Spell-Like Abilities
- *Detect Poison*
- *Goodberry*
- *Purify Food and Drink*

## Spells (Cleric List) - DC: 10 + Wis + spell lvl, CL 4
- Concentration: CL + Wis
- https://www.d20pfsrd.com/magic/spell-lists-and-domains/spell-lists-cleric
- Range bands: 35/140/560 ft

### **0th**: Prepare 4
- *Create Water*
- *Guidance*
- *Resistance*
- *Stabilize*

### **1st**: Prepare 3(+1); *Cure Light Wounds*
- *Bless*
- *Bless*
- *Protection from Evil*
- *Shield of Faith*
<!-- - *Know the Enemy* -->

### **2nd**: Prepare 1+1; *Cure Moderate Wounds*
- *Bull's Strength* (1/day double duration when cast) (+1 CL)
- *Resist Energy*
